...
volatile bool s_boot_irq_down = false;
volatile unsigned long s_boot_irq_down_ms = 0;
bool s_boot_long_down = false;
unsigned long s_boot_long_down_ms = 0;
...
void IRAM_ATTR onBootButtonIsr() {
  ...
  if (down) {
    s_boot_irq_down = true;
    s_boot_irq_down_ms = now;
  } else if (s_boot_irq_down) {
    const unsigned long held = now - s_boot_irq_down_ms;
    if (held >= config::kBootTapMinMs && held < config::kBootResetHoldMs) {
      s_boot_tap_pending = true;
    }
    s_boot_irq_down = false;
  }
...
constexpr char kPasswordInputAttrs[] = " type=\"password\"";

WiFiManagerParameter s_param_lat("radar_lat", "Latitude (deg)", "0",
                                kCoordParamLen, kCoordInputAttrs);
WiFiManagerParameter s_param_lon("radar_lon", "Longitude (deg)", "0",
                                kCoordParamLen, kCoordInputAttrs);
WiFiManagerParameter s_param_adsb_source("adsb_src", "ADS-B source (adsbfi, tar1090 or adsbx)",
                                        "adsbfi", services::adsb::kSourceNameMaxLen);
WiFiManagerParameter s_param_tar1090_url("tar1090_url", "TAR1090 aircraft.json URL",
                                        "", services::adsb::kEndpointUrlMaxLen);
WiFiManagerParameter s_param_adsbx_key("adsbx_key", "ADS-B Exchange RapidAPI key",
                                      "", services::adsb::kApiKeyMaxLen,
                                      kPasswordInputAttrs);
...
void refreshPortalParamDefaults() {
  ...
  s_param_adsb_source.setValue(services::adsb::sourceName(),
                               services::adsb::kSourceNameMaxLen);
  s_param_tar1090_url.setValue(services::adsb::tar1090Url(),
                               services::adsb::kEndpointUrlMaxLen);
  s_param_adsbx_key.setValue(services::adsb::adsbExchangeApiKey(),
                             services::adsb::kApiKeyMaxLen);
...
void onPortalParamsSaved() {
  ...
  services::adsb::saveSettingsFromPortal(s_param_adsb_source.getValue(),
                                         s_param_tar1090_url.getValue(),
                                         s_param_adsbx_key.getValue());
...
void attachPortalParams(WiFiManager& wm) {
  ...
  wm.addParameter(&s_param_adsb_source);
  wm.addParameter(&s_param_tar1090_url);
  wm.addParameter(&s_param_adsbx_key);
...
void bootButtonPollLongPress() {
  if (wifiBootButtonPressed()) {
    if (!s_boot_long_down) {
      s_boot_long_down = true;
      s_boot_long_down_ms = millis();
    }

    if (!s_long_press_handled &&
        millis() - s_boot_long_down_ms >= config::kBootResetHoldMs) {
      s_long_press_handled = true;
      Serial.println("BOOT held — resetting WiFi");
      wifiResetCredentialsAndReboot();
    }
  } else {
    s_boot_long_down = false;
