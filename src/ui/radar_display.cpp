...
bool s_grid_dirty = true;
bool s_grid_alloc_failed = false;
bool s_frame_alloc_failed = false;
unsigned long s_last_sweep_frame_ms = 0;

enum class ViewMode { kRadar, kDetail };

ViewMode s_view_mode = ViewMode::kRadar;
services::adsb::Aircraft s_selected_aircraft{};
...
void initPalette() {
  ...
  // Some panels expose BGR order; swap R/B so logical red still renders red.
...
int distSqBetween(int ax, int ay, int bx, int by) {
  const int dx = ax - bx;
  const int dy = ay - by;
  return dx * dx + dy * dy;
}
...
int measureTagBlockWidth(const services::adsb::Aircraft& plane) {
  ...
  if (plane.callsign[0] != '\0') {
    return s_draw->textWidth(plane.callsign);
  }
  return 0;
}
...
float sweepAngleDeg(unsigned long now_ms) {
  const unsigned long phase = now_ms % radar::kSweepPeriodMs;
  return (static_cast<float>(phase) * 360.0f) /
         static_cast<float>(radar::kSweepPeriodMs);
}

uint16_t sweepColorForStep(int step) {
  const int clamped = std::max(0, std::min(step, radar::kSweepSteps - 1));
  const int scaled = (clamped * 255) / std::max(1, radar::kSweepSteps - 1);

  const uint8_t r = static_cast<uint8_t>(2 + (68 * scaled) / 255);
  const uint8_t g = static_cast<uint8_t>(36 + (219 * scaled) / 255);
  const uint8_t b = static_cast<uint8_t>(12 + (92 * scaled) / 255);
  return tft.color565(r, g, b);
}

void drawSweep(unsigned long now_ms) {
  constexpr float kDegToRad = 0.01745329252f;
  const float lead_deg = sweepAngleDeg(now_ms);

  for (int i = 0; i < radar::kSweepSteps; ++i) {
    const float t =
        static_cast<float>(i) / static_cast<float>(radar::kSweepSteps - 1);
    const float deg = lead_deg - radar::kSweepArcDeg * (1.0f - t);
    const float rad = deg * kDegToRad;
    const int ex = radar::kCenterX +
                   static_cast<int>(lroundf(sinf(rad) * radar::kGridOuterRadius));
    const int ey = radar::kCenterY -
                   static_cast<int>(lroundf(cosf(rad) * radar::kGridOuterRadius));
    s_draw->drawWideLine(radar::kCenterX, radar::kCenterY, ex, ey,
                         radar::kSweepLineHalfWidth, sweepColorForStep(i));
  }
}

bool pointForAircraft(const services::adsb::Aircraft& plane, int* out_x,
                      int* out_y, bool* out_beyond_dot) {
  float dx_km = 0.0f;
  float dy_km = 0.0f;
  float dist_km = 0.0f;
  ...
  if (isInsideOuterRingKm(dist_km)) {
    latLonToScreen(plane.lat, plane.lon, out_x, out_y);
    *out_beyond_dot = false;
    return true;
  }

  if (beyondRingEdgeDotFromLatLon(plane.lat, plane.lon, out_x, out_y)) {
    *out_beyond_dot = true;
    return true;
  }
...
bool findAircraftAtPoint(int x, int y, services::adsb::Aircraft* selected) {
  const size_t n = services::adsb::aircraftCount();
  const services::adsb::Aircraft* planes = services::adsb::aircraftList();
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!pointForAircraft(planes[i], &px, &py, &beyond_dot)) {
      continue;
    }

    const int hit_radius =
        beyond_dot ? (radar::kBeyondRingDotRadiusPx + radar::scalePx(10))
                   : radar::scalePx(16);
    const int d = distSqBetween(x, y, px, py);
    if (d > hit_radius * hit_radius) {
      continue;
    }
    if (!found || d < best_dist_sq) {
      best_dist_sq = d;
      *selected = planes[i];
      found = true;
    }
...
const char* textOrUnknown(const char* text) {
  return (text != nullptr && text[0] != '\0') ? text : "UNKNOWN";
}

void drawFittedString(const char* text, int x, int y, int max_w,
                      textdatum_t datum) {
  char fitted[64];
  strncpy(fitted, textOrUnknown(text), sizeof(fitted) - 1);
  fitted[sizeof(fitted) - 1] = '\0';

  size_t len = strlen(fitted);
  while (len > 3 && s_draw->textWidth(fitted) > max_w) {
    fitted[len - 1] = '\0';
    --len;
  }
  if (strlen(fitted) < strlen(textOrUnknown(text)) && len > 3) {
    fitted[len - 3] = '.';
    fitted[len - 2] = '.';
    fitted[len - 1] = '.';
    fitted[len] = '\0';
  ...
  s_draw->drawString(fitted, x, y);
}

void drawDetailRow(const char* label, const char* value, int y) {
  constexpr int kCenter = config::kDisplayWidth / 2;
...
void drawAircraftDetail() {
  ...
  const char* callsign = textOrUnknown(s_selected_aircraft.callsign);

  char heading[24];
  snprintf(heading, sizeof(heading), "%.0f deg", s_selected_aircraft.nose_deg);

  char speed[24];
  if (s_selected_aircraft.gs_knots > 0.0f) {
    snprintf(speed, sizeof(speed), "%.0f kt", s_selected_aircraft.gs_knots);
  } else {
    strncpy(speed, "UNKNOWN", sizeof(speed) - 1);
  ...
  tft.drawFastHLine(32, 89, config::kDisplayWidth - 64, radar::kColorGrid);

  drawDetailRow("CALLSIGN", callsign, 126);
...
bool ensureSprite(LGFX_Sprite& sprite, bool* ready, bool* alloc_failed,
                  const char* label) {
  if (*ready) {
    return true;
  }
  ...
  if (!sprite.createSprite(radar::kSize, radar::kSize)) {
    *alloc_failed = true;
    Serial.printf("radar: %s sprite alloc failed (%dx%d, PSRAM %s, free heap %u, free PSRAM %u)\n",
                  label, radar::kSize, radar::kSize,
  ...
  *ready = true;
  Serial.printf("radar: %s sprite allocated in %s (%dx%d)\n", label,
                psramFound() ? "PSRAM" : "heap", radar::kSize,
                radar::kSize);
  return true;
}
...
bool ensureFrameSprite() {
  const bool frame_ok =
      ensureSprite(s_frame, &s_frame_ready, &s_frame_alloc_failed, "frame");
  const bool grid_ok =
      ensureSprite(s_grid, &s_grid_ready, &s_grid_alloc_failed, "grid");
  return frame_ok && grid_ok;
}
...
void radarDisplayRefreshAircraft() {
  if (s_view_mode == ViewMode::kDetail) {
    drawAircraftDetail();
    return;
  }
...
bool radarDisplayAnimate() {
  if (s_view_mode != ViewMode::kRadar) {
    return false;
  }
...
bool radarDisplayHandleTouch(int x, int y) {
  ...
  services::adsb::Aircraft selected{};
  if (!findAircraftAtPoint(x, y, &selected)) {
    return false;
  }

  s_selected_aircraft = selected;
  s_view_mode = ViewMode::kDetail;
  drawAircraftDetail();
  return true;
}
