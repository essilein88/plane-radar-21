...
constexpr int scalePx(int px) {
  return (px * config::kDisplayHeight + 120) / 240;
}

constexpr int kLineGap = scalePx(6);
const int kCenterX = config::kDisplayWidth / 2;
const int kCenterY = config::kDisplayHeight / 2;

constexpr int kSpinnerDotCount = 10;
constexpr int kSpinnerRadius = (config::kDisplayHeight / 2) - scalePx(7);
constexpr int kSpinnerDotRadius = scalePx(2);
constexpr int kSpinnerEraseRadius = scalePx(4);
...
char s_ssid_line[33];
constexpr int kConnectingTextMaxWidthPx = config::kDisplayWidth - scalePx(20);
float s_spinner_angle_deg = -90.0f;
SpinnerDot s_spinner_dots[kSpinnerDotCount];
bool s_connecting_text_drawn = false;

constexpr auto& kGfxTitle = lgfxfonts::FreeSans18pt7b;
constexpr auto& kGfxBody = lgfxfonts::FreeSans12pt7b;
constexpr auto& kGfxDetail = lgfxfonts::Font2;
constexpr auto& kPortalGfxTitle = lgfxfonts::FreeSansBold18pt7b;
constexpr auto& kPortalGfxBody = lgfxfonts::FreeSansBold12pt7b;
constexpr auto& kPortalGfxEmphasis = lgfxfonts::FreeSansBold18pt7b;
constexpr auto& kConnectingGfxDetail = lgfxfonts::FreeSans9pt7b;
...
void drawConnectingText() {
  ...
  constexpr int kPanelPadY = scalePx(8);
